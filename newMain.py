...
parser.add_argument('-e', dest="epoch_number", default=100,
                    help="Number of Epochs to train the model", type=int)
parser.add_argument('-v', dest="Vis_step", default=1000,
                    help="at every Vis_step 'minibatch' the plots will be updated")
parser.add_argument('-redraw', dest="redraw", default=False,
                    help="either update the log plot each step")
parser.add_argument('-lr', dest="lr", default=0.0003,
...
parser.add_argument('-graphEmDim', dest="graphEmDim", default=1024,
...
parser.add_argument('-f', dest="use_feature", default=True,
...
parser.add_argument('-decoder', dest="decoder", default="FC",
                    help="the decoder type, FC is only option in this rep")
parser.add_argument('-encoder', dest="encoder_type", default="DynamicCouplingEncoder",
                    help="the encoder: only option in this rep is 'AvePool'")  # only option in this rep is "AvePool"
parser.add_argument('-batchSize', dest="batchSize", default=32,
                    help="the size of each batch; the number of graphs is the mini batch")
parser.add_argument('-UseGPU', dest="UseGPU", default=True,
...
parser.add_argument('-device', dest="device", default="cuda:0",
                    help="Which device should be used")
# parser.add_argument('-task', dest="task", default="graphGeneration", help="only option in this rep is graphGeneration")
# MODIFICATION 1: Changed default to False
parser.add_argument('-BFS', dest="bfsOrdering", default=False,
                    help="use bfs for graph permutations", type=bool)
parser.add_argument('-directed', dest="directed", default=True,
                    help="is the dataset directed?!", type=bool)
parser.add_argument('-beta', dest="beta", default=None,
                    help="beta coefiicieny", type=float)
parser.add_argument('-plot_testGraphs', dest="plot_testGraphs", default=True, help="shall the test set be printed",
                    type=float)
parser.add_argument('-ideal_Evalaution', dest="ideal_Evalaution", default=False,
                    help="if you want to comapre the 50%50 subset of dataset comparision?!", type=bool)

parser.add_argument('-num_views', dest="num_views", default=2,
                    help="Number of views in the multi-view graph dataset", type=int)
parser.add_argument('-task', dest="task", default="graphClassification",
                    help="options: graphGeneration, graphClassification")
...
logging.basicConfig(filename=graph_save_path + 'log.log',
...
if args.model == "KernelAugmentedWithTotalNumberOfTriangles" or args.model == "GraphVAE-MM":
    kernl_type = ["trans_matrix", "in_degree_dist",
                  "out_degree_dist", "TotalNumberOfTriangles"]
    if dataset == "mnist":
...
class NodeUpsampling(torch.nn.Module):
    def __init__(self, InNode_num, outNode_num, InLatent_dim=None):
        ...
        self.Node_mlp = torch.nn.Linear(
            InNode_num * InLatent_dim, InLatent_dim * outNode_num)

    def forward(self, inTensor, activation=torch.nn.LeakyReLU(0.001)):
        Z = self.Node_mlp(inTensor.reshape(inTensor.shape[0], -1))
        Z = torch.matmul(
            Z.reshape(inTensor.shape[0], -1).permute(0, 2, 1), inTensor)
...
class LatentMtrixTransformer(torch.nn.Module):
    def __init__(self, InNode_num, InLatent_dim=None, OutLatentDim=None):
        super(NodeUpsampling, self).__init__()
        self.Node_mlp = torch.nn.Linear(
            InNode_num * InLatent_dim, InNode_num * OutLatentDim)

    def forward(self, inTensor, activation=torch.nn.LeakyReLU(0.001)):
        Z = self.Node_mlp(inTensor.reshape(inTensor.shape[0], -1))
        Z = torch.matmul(inTensor, Z.reshape(inTensor.shape[-1], -1))
...
def test_(number_of_samples, model, graph_size, path_to_save_g, remove_self=True, save_graphs=True):
    ...
    for g_size in graph_size:
        for j in range(number_of_samples):
            z = torch.tensor(np.random.normal(size=[1, model.embeding_dim]))
            ...
            if save_graphs:
                plotter.plotG(G, "generated" + dataset,
                              file_name=f_name + "_ConnectedComponnents")
    # ======================================================
    # save nx files
    if save_graphs:
        nx_f_name = path_to_save_g + "_" + dataset + "_" + \
            decoder_type + "_" + args.model + "_" + task
...
def EvalTwoSet(model, test_list_adj, graph_save_path, Save_generated=True, _f_name=None, onlyTheBigestConCom=True):
    generated_graphs = test_(1, model, [
                             x.shape[0] for x in test_list_adj], graph_save_path, save_graphs=Save_generated)
    ...
    if (onlyTheBigestConCom == False):
        if Save_generated:
            ...
            logging.info(mmd_eval(generated_graphs, [nx.from_numpy_array(
                graph.toarray()) for graph in test_list_adj]))
    print("====================================================")
    logging.info("====================================================")
    ...
    statistic_ = mmd_eval(generated_graphs, [nx.from_numpy_array(
        graph.toarray()) for graph in test_list_adj], diam=True)
    # if writeThem_in!=None:
    #     with open(writeThem_in+'MMD.log', 'w') as f:
    #         f.write(statistic_)
    logging.info(statistic_)
    if Save_generated:
        graphs_to_writeOnDisk = [
            nx.to_numpy_array(G) for G in generated_graphs]
        ...
        np.save(graph_save_path + 'testGraphs_adj_.npy',
                graphs_to_writeOnDisk, allow_pickle=True)
    return statistic_
...
def OptimizerVAE(reconstructed_adj_logit_views, reconstructed_kernel_val_views,
    ...
    target_adj_views_sliced = target_adj_views[:,
                                               :, :max_nodes_in_batch, :max_nodes_in_batch]
    ...
    if target_kernel_val_views and reconstructed_kernel_val_views and kernl_type:
        ...
        for v in range(num_views):
            ...
            for i in range(len(target_kernels_v)):
                log_sigma = (
                    (recon_kernels_v[i] - target_kernels_v[i]) ** 2).mean().sqrt().log()
                log_sigma = softclip(log_sigma, -6)
                step_loss = log_guss(
                    target_kernels_v[i], log_sigma, recon_kernels_v[i]).mean()
                # Use the alpha weights for the kernel terms
                weighted_step_loss = step_loss * alpha[i]
                kernel_loss += weighted_step_loss
                each_kernel_loss.append(
                    weighted_step_loss.cpu().detach().numpy())

    # --- 3. KL Divergence ---
    # This is the standard KL divergence formula for a VAE, which is positive.
    kl_norm_factor = mean.shape[0] * mean.shape[1]
    kl_loss = (1 / kl_norm_factor) * -0.5 * torch.sum(1 + 2 *
                                                      log_std - mean.pow(2) - torch.exp(log_std).pow(2))
...
list_adj, list_x, list_label = list_graph_loader(
    args.dataset, return_labels=True)  # , _max_list_size=80)
...
if ideal_Evalaution:
    ...
    fifty_fifty_dataset = [nx.from_numpy_array(
        graph.toarray()) for graph in fifty_fifty_dataset]
    random.shuffle(fifty_fifty_dataset)
    print("50%50 Evalaution of dataset")
    logging.info(mmd_eval(fifty_fifty_dataset[:int(len(
        fifty_fifty_dataset)/2)], fifty_fifty_dataset[int(len(fifty_fifty_dataset)/2):], diam=True))
...
print(
    f"--- Data Processed. Determined input feature dimension: {in_feature_dim} ---")
...
degree_width = torch.tensor([[.1] for x in range(0, SubGraphNodeNum, 1)])
# ToDo: both bin's center and widtg also maximum value of it should be determinde auomaticly
...
if encoder_type == "DynamicCouplingEncoder":
...
print(
    f"KL Annealing will occur over the first {kl_anneal_epochs} epochs ({kl_anneal_steps} steps).")
# ------------------------------------
...
for epoch in range(epoch_number):
    ...
    for iter in range(0, max(int(len(list_graphs.list_adjs) / mini_batch_size), 1) * mini_batch_size, mini_batch_size):
        ...
        features_for_dgl = x_s_tensor.reshape(
            total_nodes, feature_dim).to(device)
        ...
        batchSize_for_encoder = [len(org_adj), org_adj[0].shape[1]]

        dgl_graphs_per_view = []
        for v in range(args.num_views):
            view_graphs_in_batch = [dgl.from_scipy(
                sp.csr_matrix(g[v].cpu().numpy())) for g in org_adj]
            dgl_graphs_per_view.append(
                dgl.batch(view_graphs_in_batch).to(device))
        ...
        if (step + 1) % visulizer_step == 0 or epoch_number == epoch+1:
            ...
            if True:
                ...
                reconstructed_first_view = torch.sigmoid(
                    reconstructed_adj_logit)[:, 0, :, :]

                # Now select a random sample from that view
                sample_graph = reconstructed_first_view[rnd_indx].cpu(
                ).detach().numpy()
                sample_graph = sample_graph[:node_num[rnd_indx],
                                            :node_num[rnd_indx]]
                sample_graph[sample_graph >= 0.5] = 1
                sample_graph[sample_graph < 0.5] = 0
                ...
                reconstructed_graphs = [nx.from_numpy_array(
                    reconstructed_adj_for_eval[i]) for i in range(reconstructed_adj_for_eval.shape[0])]
                ...
                target_set = [nx.from_numpy_array(val_adj[i][0].toarray()) for i in range(
                    len(val_adj))]  # Using first view of validation set
                target_set = [nx.Graph(G.subgraph(max(nx.connected_components(G), key=len))) for G in target_set if
                              not nx.is_empty(G)]

                reconstruc_MMD_loss = mmd_eval(
                    reconstructed_graphs, target_set[:len(reconstructed_graphs)], diam=True)
        ...
        # scheduler.step()
model.eval()
torch.save(model.state_dict(), graph_save_path +
           "model_" + str(epoch) + "_" + str(batch))
...
file_name = graph_save_path + "_" + encoder_type + "_" + decoder_type + \
    "_" + dataset + "_" + task + "_" + args.model + "_elbo_loss.txt"

with open(file_name, "w") as fp:
    json.dump(
        list(np.array(pltr.values_train[-2]) + np.array(pltr.values_train[-1])), fp)
...
if task == "graphGeneration":
    EvalTwoSet(model, test_list_adj, graph_save_path,
               Save_generated=True, _f_name="final_eval")
...
if args.task == "graphClassification":
    ...
    train_labels = torch.tensor(
        np.array(list_graphs.labels), dtype=torch.float32).to(device)
    test_labels = torch.tensor(
        np.array(list_test_graphs.labels), dtype=torch.float32).to(device)
    ...
    logging.info(
        "Generating embeddings and preparing DGL graphs for train and test sets...")
    model.eval()
    with torch.no_grad():
        # --- Get Train Data ---
        train_dgl_views = []
        for v in range(args.num_views):
            scipy_graphs = [sp.csr_matrix(g[v].cpu().numpy())
                            for g in list_graphs.adj_s]
        ...
        train_features = torch.stack(
            list_graphs.x_s).reshape(-1, list_graphs.feature_size).to(device)
        train_batch_size = [len(list_graphs.adj_s), list_graphs.max_num_nodes]
        train_mean_nodes, _ = model.encode(
            train_dgl_views, train_features, train_batch_size)
        train_graph_vectors = []
        ...
        for v in range(args.num_views):
            scipy_graphs = [sp.csr_matrix(g[v].cpu().numpy())
                            for g in list_test_graphs.adj_s]
            dgl_graphs = [dgl.from_scipy(sg) for sg in scipy_graphs]
            test_dgl_views.append(dgl.batch(dgl_graphs).to(device))
        test_features = torch.stack(
            list_test_graphs.x_s).reshape(-1, list_test_graphs.feature_size).to(device)
        test_batch_size = [len(list_test_graphs.adj_s),
                           list_test_graphs.max_num_nodes]
        test_mean_nodes, _ = model.encode(
            test_dgl_views, test_features, test_batch_size)
        test_graph_vectors = []
    ...
    print(
        f"Generated {train_embeddings.shape[0]} training embeddings (dim={train_embeddings.shape[1]}) and {test_embeddings.shape[0]} test embeddings.")
    logging.info(
        f"Generated {train_embeddings.shape[0]} training embeddings (dim={train_embeddings.shape[1]}) and {test_embeddings.shape[0]} test embeddings.")
    ...
    diffusion_classifier = LabelDiffusionClassifier(
        graph_embedding_dim=diffusion_classifier_embedding_dim, DDM_config=ddm_config).to(device)
    print("Skipping torch.compile() due to C++ compilation error with DGL.")

    optimizer = torch.optim.Adam(
        diffusion_classifier.parameters(), lr=1e-4, weight_decay=1e-5)
    ...
    for epoch in range(num_diffusion_epochs):
        ...
        for i in range(0, len(train_indices), batch_size):
            ...
            with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                loss, _ = diffusion_classifier(
                    batch_dgl_graph, batch_embeddings, batch_labels)
