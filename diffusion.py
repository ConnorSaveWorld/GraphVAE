...
# from utils import make_edge_weights
...
class DDM(nn.Module):
    ...
    def sample_q(self, t, x, g):
        miu, std = x.mean(dim=0), x.std(dim=0)+ 1e-5
    ...
    def embed(self, g, x, T):
        ...
        # Generate noisy data and time embedding
        x_t, time_embed, g = self.sample_q(t, x, g)

        # Process data through the network
        _, hidden = self.net(g, x_t=x_t, time_embed=time_embed)

        # # Normalize output embeddings for better discrimination
...
def loss_fn(x, y, alpha=2):
    ...
    out_norm = F.normalize(x, p=2, dim=-1)
    cos_loss = (1 - (out_norm * x_norm).sum(dim=-1)).pow_(alpha).mean()

    # Combine losses with a weight parameter
...
def get_beta_schedule(beta_schedule, beta_start, beta_end, num_diffusion_timesteps):
    ...
    elif beta_schedule == "cosine":  # Cosine schedule from improved DDPM paper
